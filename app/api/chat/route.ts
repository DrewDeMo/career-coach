...
// Available models (as of current OpenAI API documentation)
...
function validateModel(model: string): string {
    // If model is in available list, use it
    if (AVAILABLE_MODELS.includes(model)) {
        return model
    }

    // Check if it's a model variant that might work
    if (model.startsWith('gpt-')) {
        console.warn(`Model ${model} not in known list, attempting to use it anyway`)
        return model
    }

    // Fall back to default
    console.warn(`Model ${model} not recognized, falling back to ${MODEL}`)
    return MODEL
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { message, conversationId, conversationHistory, stream = true, model } = await request.json()
        ...
        if (stream) {
            ...
            // Use model from request if provided, otherwise use environment variable
            const selectedModel = validateModel(model || MODEL)
